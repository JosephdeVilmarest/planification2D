...
def scanning(lobstacles,LEN):
    ...
    for c in cells:
        if c.xMax == c.xMin:
            for i in c.previousCells:
                i.nextCells.extend(c.nextCells)
                i.nextCells.remove(c)
            for i in c.nextCells:
                i.previousCells.extend(c.previousCells)
                i.previousCells.remove(c)
            c.nextCells = []
            c.previousCells = []
    return cells
